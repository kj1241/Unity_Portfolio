...
public class SceneControl : MonoBehaviour {
    ...
    void OnGUI()
    {
        switch (this.step)
        {
            ...
            case STEP.CLEAR:
                ...
                // 클리어 시간을 표시.
              //  GUI.Label(new Rect(Screen.width / 2.0f - 80.0f, 40.0f, 200.0f, 20.0f), "클리어 시간" + Mathf.CeilToInt(this.clear_time).ToString() + "초", guistyle);
                GUI.color = Color.white;
                break;
        }
    }
    ...
    void Start () {
        ...
        this.guistyle.fontSize = 24; // 폰트 크기
        ...
        if (this.next_step == STEP.NONE)
        {
            switch (this.step)
            {
                case STEP.PLAY:
                    ...
                    {
                        foreach (BlockControl block in BlockRoot.blocks)
                        {
                            Global.blackBump = true;
                            block.bump = true;
                            BlockRoot.checkConnection(block);
                        }
                        Invoke("ClearSence", 3);
                        // 클리어 상태로 이행.
                    }
