...
//특성2. 콤보시스템 == 피버 
//특성3 종료조건 정해진시간으로 게임 종료


public class BlockRoot : MonoBehaviour
{
    public GameObject BlockPrefab = null; // 만들어낼 블록의 프리팹. 
   static public BlockControl[,] blocks; // 그리드.
            ...
            if (!this.is_has_falling_block())
            {
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    foreach (BlockControl block in blocks)
                    {
                        ...
                        if (grabbed_block.color == (Block.COLOR)10)
                        {
                            SoundManager.instance.PlayEffect2();
                            grabbed_block.rewindVanishTimer();
                            Global.blackBump = true;
                       
                        }
                        if (grabbed_block.color == (Block.COLOR)11)
                        {
                            SoundManager.instance.PlayEffect3();
                            Global.redBump = true;
                            checkConnection(block);
                        }
                        break;
                    }
                }
                ...
                else
                {
                    ...
                    {
                        block.bump = false;
                        Global.blackBump = false;
                        Global.redBump=false;
                    }
                }
            }
            ...
            for (int x = 0; x < Block.BLOCK_NUM_X; x++)
            {
                ...
                for (int y = 0; y < Block.BLOCK_NUM_Y - 1; y++)
                {
                    ...
                    for (int y1 = y + 1; y1 < Block.BLOCK_NUM_Y; y1++)
                    {
                        ...
                        // 블록을 교체한다.
                        this.fallBlock(blocks[x, y], Block.DIR4.UP,
                        blocks[x, y1]);
                        break;
                    }
                }
    ...
    static public bool checkConnection(BlockControl start)
    {
        bool ret = false;
        ...
        if(blocks[start.i_pos.x, start.i_pos.y].color == (Block.COLOR)11&&Global.redBump==true)
        {
            foreach (BlockControl block in blocks)
            {
        ...
        if (blocks[start.i_pos.x, start.i_pos.y].color == (Block.COLOR)10&&Global.blackBump==true)
        {
      
            blocks[start.i_pos.x, start.i_pos.y].rewindVanishTimer();
            blocks[start.i_pos.x-1, start.i_pos.y].rewindVanishTimer();
            blocks[start.i_pos.x, start.i_pos.y-1].rewindVanishTimer();
            blocks[start.i_pos.x-1, start.i_pos.y-1].rewindVanishTimer();
            // blocks[start.i_pos.x-1, start.i_pos.y+1].rewindVanishTimer();
            //blocks[start.i_pos.x, start.i_pos.y+1].rewindVanishTimer();
            //blocks[start.i_pos.x+1, start.i_pos.y+1].rewindVanishTimer();
            //blocks[start.i_pos.x+1, start.i_pos.y].rewindVanishTimer();
        ...
        {
            if (blocks[start.i_pos.x, start.i_pos.y].bump == false)
                break;
    ...
    { // 그리드의 크기를 9×9로 한다. 
        blocks = new BlockControl[Block.BLOCK_NUM_X, Block.BLOCK_NUM_Y]; // 블록의 색 번호. 
        int color_index = 0;
        int item_index = 0;
        Block.COLOR color = Block.COLOR.FIRST;
        ...
        { // 처음~마지막행 
            for (int x = 0; x < Block.BLOCK_NUM_X; x++)
            { // 왼쪽~오른쪽 // BlockPrefab의 인스턴스를 씬에 만든다.
                GameObject game_object = Instantiate(this.BlockPrefab) as GameObject; // 위에서 만든 블록의 BlockControl 클래스를 가져온다.
                ...
                if (x>1)
                {
                    if (blocks[x - 2, y].color == blocks[x - 1, y].color && blocks[x - 1, y].color == blocks[x, y].color)
                        //if (blocks[x - 2, y].color != (Block.COLOR)10 && blocks[x - 1, y].color != (Block.COLOR)10)
//                            if (blocks[x - 2, y].color != (Block.COLOR)11 && blocks[x - 1, y].color != (Block.COLOR)11)
                                block.setColor((Block.COLOR)item_index);
                }
                if (y > 1)
                {
                    if (blocks[x , y-2].color == blocks[x , y-1].color && blocks[x , y-1].color == blocks[x, y].color)
                       // if (blocks[x, y-2].color != (Block.COLOR)10 && blocks[x, y-1].color != (Block.COLOR)10)
//                            if (blocks[x - 2, y].color != (Block.COLOR)11 && blocks[x - 1, y].color != (Block.COLOR)11)
                                block.setColor((Block.COLOR)item_index);
                }
                ...
                block.name = "block(" + block.i_pos.x.ToString() + "," + block.i_pos.y.ToString() + ")"; // 전체 색 중에서 임의로 하나의 색을 선택한다.
                color_index = Random.Range(0, (int)Block.COLOR.NORMAL_COLOR_NUM);
                item_index = Random.Range(10, 12);
            }
        }
