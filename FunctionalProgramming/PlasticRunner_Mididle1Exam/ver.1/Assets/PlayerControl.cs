...
public class PlayerControl : MonoBehaviour
{
    ...
    public enum STEP
    { // Player        의 각종 상태를 나타내는 자료형 (* 열거체        )
        NONE = -1, // 상태정보 없음.
        RUN = 0, // 달린다.
        JUMP, // 점프.
        D_JUMP,
        MISS, // 실패.
        NUM, // 상태가몇 종류 있는지 보여준다(=3).
    };

    public STEP step = STEP.NONE; // Player    의 현재 상태    .
    public STEP next_step = STEP.NONE; // Player    의 다음 상태    .
        ...
        while (this.next_step != STEP.NONE)
        {
            ...
            { // 갱신된 '현재 상태'가.
                case STEP.JUMP: // '점프'일 때.
                    ...
                    break;
                case STEP.D_JUMP:
                    velocity.y = Mathf.Sqrt(1.0f * 9.8f * PlayerControl.JUMP_HEIGHT_MAX);
                    this.is_key_released = false;
                    break;
            }
