...
namespace FireBall
{
    public delegate void FireBaltCollisionDelegate(FireBaltSystem script, Vector3 pos); //파이어볼 콜라이더 델리게이트
    public class FireBaltSystem : SkillScript, ICollisionHandler
    {

        [Header("FireBaltSystem")]
        ...
        [HideInInspector]
        public FireBaltCollisionDelegate CollisionDelegate; //델리게이트
        ...
        private IEnumerator ParticleMove()
        {
            ...
            {
                MoveObject.transform.position = ColliderObject.transform.position;
        ...
        public void HandleCollision(GameObject obj, Collision c) //인터페이스 상속받은 콜라이더 충돌시
        {
            ...
            {
                foreach (ParticleSystem o in DestroyParticleSystemsOnCollision)
                {
                    GameObject.Destroy(o, 0.01f);// 파티클 제거
                }
            }
